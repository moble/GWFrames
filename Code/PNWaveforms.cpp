...
GWFrames::PNWaveform::PNWaveform(const std::string& Approximant, const double delta,
				 ...
				 const double Omega_orb_0, const Quaternions::Quaternion& R_0) :
  Waveform(), mchi1(0), mchi2(0), mOmega_orb(0), mOmega_prec(0), mL(0), mPhi_orb(0)
{
  ///
  ...
  const Quaternions::Quaternion chi1_0Q = R_0 * Quaternions::Quaternion(chi1_0) * R_0.conjugate();
  const Quaternions::Quaternion chi2_0Q = R_0 * Quaternions::Quaternion(chi2_0) * R_0.conjugate();
  const Quaternions::Quaternion LNHatQ  = R_0 * zHat * R_0.conjugate();
  const Quaternions::Quaternion Rax = Quaternions::sqrtOfRotor(-LNHatQ*zHat);
  ...
  ystart[11] = Quaternions::angle(Rax.conjugate()*R_0);  // gamma
  ...
  TaylorTn Tn((1+delta)/2., ystart[0],
	      chi1_0[0], chi1_0[1], chi1_0[2],
	      chi2_0[0], chi2_0[1], chi2_0[2],
	      0.,0.,1.,
	      1.,0.,0.);
  Tn.Recalculate(time, &ystart[0]);
  // Tn.RecalculateValues(time, &ystart[0]);
  ...
  const Quaternions::Quaternion zHat(0,0,0,1);
